...
export const createWorkspace = async (req: Request, res: Response) => {
  try {
    ...
    const workspace = await workspaceService.createWorkspace(userId, name, image);
...
export const createInviteLink = async (req: Request, res: Response) => {
  try {
    ...
    const invite = await workspaceService.createInviteLink(id, userId, maxUses, expiresInDays);
...
export const joinWorkspace = async (req: Request, res: Response) => {
  try {
    ...
    const workspace = await workspaceService.joinWorkspace(code, userId);
    return res.status(200).json({ workspace });
  } catch (error: any) {
